  ...
  return (
    <div className="card shadow-sm">
      <div className="card-header bg-primary text-white d-flex align-items-center justify-content-between">
        <div>
          <i className="fa fa-exchange-alt mr-2"></i>
          <h6 className="mb-0 d-inline">College-wise Student Fee Transactions</h6>
        </div>
      ...
      <div className="card-body p-0">
        {transactions.length === 0 ? (
          <div className="text-center py-5">
            ...
            <p className="text-muted">No fee records available for your college.</p>
          </div>
        ) : (
  ...
  />
              ...
              <tbody>
                {transactions.map((transaction, index) => {
                  const balance = (transaction.amountDue || 0) - (transaction.paid || 0);
                  return (
                    <tr key={`${transaction.studentId}-${transaction.hid}-${index}`}>
                      ...
                      <td>{getStatusBadge(transaction.paid, transaction.amountDue)}</td>
                      <td>
          ...
          <div className="row">
            <div className="col-md-3">
              <small className="text-muted">
                ...
                Total Students: {new Set(transactions.map(t => t.studentId)).size}
              </small>
            </div>
            <div className="col-md-3">
              <small className="text-muted">
                <i className="fa fa-money-bill-wave mr-1"></i>
                Total Due: {formatCurrency(transactions.reduce((sum, t) => sum + (t.amountDue || 0), 0))}
              </small>
            </div>
            <div className="col-md-3">
              <small className="text-muted">
                <i className="fa fa-check-circle mr-1"></i>
                Total Paid: {formatCurrency(transactions.reduce((sum, t) => sum + (t.paid || 0), 0))}
              </small>
            </div>
            <div className="col-md-3">
